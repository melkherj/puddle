...
\Require label oracle $\mathcal{O}:\mathcal{E}\mapsto\{0..c\}$ \Comment{c the number of classes}
...
\State $\theta \leftarrow \mathcal{N}(0,1)^{k}$ \Comment{Initialize all model parameters,k is num params}
...
\State 
\State //Active
\If{rand()<$\eta_1$} \Comment{Active sample every few epochs}
\State sample $\delta_1 \sim  \mathcal{N}(0,\alpha e^{-\beta | A | })$ \Comment{added noise to sampling score}
...
\State $pos \leftarrow (1-\gamma)\mathbbm{1}\{predict(i)=Y_i\} + \gamma pos$ \Comment{Adjust count of positives}
...
\State $B \leftarrow \left\{b \mid model(b)> \tau_s \right\}$ \Comment{Select examples to pseudo-label}
\For{$i \leftarrow B$}
\State $Y_i \leftarrow predict_{\theta} (i)$  \Comment{Do pseudo-labeling}
\EndFor
\State
\State //Model Update
\State $a \leftarrow \textrm{balanced-sample}(A,40)$
\State $b \leftarrow \textrm{balanced-sample}(B,40)$
\State $\theta \leftarrow \textrm{minibatch-fit}(X,Y,a \cup b)$ \Comment{Update model using real/pseudo-labels}
\State
\State //Rescore
